#!/bin/sh
# from sidecar container blog post https://blog.openshift.com/kerberos-sidecar-container

# allow the period to be configurable, the default is 1 hour
[[ "$PERIOD_SECONDS" == "" ]] && PERIOD_SECONDS=3600

# allow for specific kinit options to be provided, but if not tehse are inferred based on provided keytab files
if [[ "$OPTIONS" == "" ]]; then
